                ...
                if (hasPrefixIdColumn) {
                    ...
                    db.get("SELECT sql FROM sqlite_master WHERE type='table' AND name='topics'", (err, result) => {
                        ...
                        db.serialize(() => {
                            ...
                            // Check if auto_use_device_name column exists, add it if not
                            const hasAutoUseDeviceName = rows.some(col => col.name === 'auto_use_device_name');
                            if (!hasAutoUseDeviceName) {
                                db.run(`ALTER TABLE topics ADD COLUMN auto_use_device_name INTEGER DEFAULT 0`, (err) => {
                                    if (err) {
